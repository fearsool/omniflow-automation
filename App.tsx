...
import { AutomationPackager } from './components/AutomationPackager';
...
const App: React.FC = () => {
  ...
  const [showResults, setShowResults] = useState(false); // Results Panel
  const [showHistory, setShowHistory] = useState(false); // History Panel
  const [showCarousel, setShowCarousel] = useState(false); // Carousel Generator
  const [automationHistory, setAutomationHistory] = useState<Array<{ id: string; name: string; timestamp: string; results: any; status: string }>>([]);
